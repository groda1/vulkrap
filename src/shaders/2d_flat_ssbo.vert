...
void main() {
    ...
    vec2 model_position = text_data.instances[gl_InstanceIndex].position;
    vec2 size = text_data.instances[gl_InstanceIndex].size;

    vec4 position = vec4((inPosition.x * size.x) + model_position.x, (inPosition.y *size.y) + model_position.y, 0.0, 1.0);
