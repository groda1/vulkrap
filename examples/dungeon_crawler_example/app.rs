...
use crate::dungeon_crawler_example::movement::{Movement, MovementInput, Orientation, RotationalInput};
...
impl VulkrapApplication for DungeonCrawler {
    ...
    fn handle_keyboard_event(&mut self, context: &mut Context, key: VirtualKeyCode, state: ElementState) -> ControlSignal {
        ...
        match (key, state) {
            (VirtualKeyCode::W, ElementState::Pressed) => self.movement.add_move_input(MovementInput::Forward),
            (VirtualKeyCode::S, ElementState::Pressed) => self.movement.add_move_input(MovementInput::Backward),
            (VirtualKeyCode::A, ElementState::Pressed) => self.movement.add_move_input(MovementInput::Left),
            (VirtualKeyCode::D, ElementState::Pressed) => self.movement.add_move_input(MovementInput::Right),
            (VirtualKeyCode::Q, ElementState::Pressed) => self.movement.add_rot_input(RotationalInput::Left),
            (VirtualKeyCode::E, ElementState::Pressed) => self.movement.add_rot_input(RotationalInput::Right),
...
impl DungeonCrawler {
    pub fn new(context: &mut Context, engine_params: EngineParameters) -> DungeonCrawler {
        ...
        let start_position = Vector2::new(start_x, start_y);
